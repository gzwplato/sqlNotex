...
type
  ...
  published
    ...
    class function SetCursorMiddleScreen(const AWinControl: TWinControl; 
      CurrLine: integer): boolean; override;
    class function SetSpaces(const AWinControl: TWinControl; 
      LineSp, ParSp, RightSp: Integer): boolean; override;
...
class function TGtk2WSCustomRichMemo.SetSpaces(
  const AWinControl: TWinControl; LineSp, ParSp, RightSp: Integer): boolean;
var
  Widget, TextWidget: PGtkWidget;
  list: PGList;
begin
  ...
  if not Assigned(TextWidget) then
    Exit;
  gtk_text_view_set_pixels_inside_wrap(PGtkTextView(TextWidget), LineSp);
  gtk_text_view_set_pixels_below_lines(PGtkTextView(TextWidget), ParSp);
  gtk_text_view_set_right_margin(PGtkTextView(TextWidget), RightSp);
end;
